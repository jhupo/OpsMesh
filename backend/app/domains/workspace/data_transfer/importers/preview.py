...
from backend.app.domains.workspace.data_transfer.importers.context import _int_from_optional_string
...
def _validated_checksum(
    *,
    ...
    if not matched:
        response.warnings.append(f"Skipped {collection[:-1]} {source_id}: checksum mismatch")
