...
from backend.app.domains.workspace.data_lifecycle.queries import (
    WorkspaceDataLifecycleDiagnosticQueries,
    archive_integrity_payload,
    audit_event_payload,
    export_job_payload,
)
...
from backend.app.domains.workspace.data_lifecycle.scheduling import (
    automation_backup_warnings,
    automation_restore_drill_warnings,
    automation_retention_warnings,
...
class WorkspaceLifecycleDiagnosticsService:
    ...
    def get_diagnostics(self, *, workspace_id: UUID) -> dict[str, object] | None:
        ...
        return {
            ...
            "export_import": {
                ...
                "latest_export_job": export_job_payload(latest_job),
                "latest_successful_archive_export": export_job_payload(latest_success),
            },
    ...
    def get_recovery_readiness(self, *, workspace_id: UUID) -> dict[str, object] | None:
        ...
        archive_integrity = archive_integrity_payload(
            latest_integrity,
            latest_success=latest_success,
        )
        ...
        return {
            ...
            "generated_at": generated_at,
            "latest_successful_archive_export": export_job_payload(latest_success),
            "latest_archive_import": audit_event_payload(latest_import),
            "latest_restore_drill": audit_event_payload(latest_restore_drill),
            "latest_failed_export_job": export_job_payload(latest_failed_job),
    ...
    def _automation_diagnostics(
        ...
        *,
        ...
        retention_interval_hours = backup_interval_hours(raw_retention)
        restore_drill_interval_hours = backup_interval_hours(raw_restore_drill)
        ...
        restore_drill_next_due_at = (
            ...
            if latest_restore_drill_at is not None and restore_drill_interval_hours is not None
            else None
        )
        active_archive_export_count = self._repo.active_archive_export_job_count(workspace.id)
        backup_due = scheduled_backup_due(backup_policy)
        restore_due = restore_drill_due(
        ...
        return {
            "scheduled_backup": {
                ...
                "configured": schedule_configured(backup_policy),
                "due": backup_due,
                "blocked_by_active_export": backup_due and active_archive_export_count > 0,
                "active_archive_export_job_count": active_archive_export_count,
                "latest_scheduled_archive_export_job": export_job_payload(
                    self._repo.latest_scheduled_archive_export_job(workspace.id)
                ),
                "latest_event": audit_event_payload(
                ...
                "warnings": automation_backup_warnings(
                    backup_policy,
            ...
            "scheduled_retention": {
                ...
                "warnings": automation_retention_warnings(
            ...
            "scheduled_restore_drill": {
                ...
                "due": restore_due,
                "blocked_by_active_export": restore_due and active_archive_export_count > 0,
                ...
                "warnings": automation_restore_drill_warnings(
                    raw_policy=raw_restore_drill,
                    interval_hours=restore_drill_interval_hours,
                    latest_success=latest_success,
                    latest_drill=latest_restore_drill,
