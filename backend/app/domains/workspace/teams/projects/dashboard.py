    ...
    def get_dashboard(
        ...
        *,
        ...
        task_statement = select(Task).where(
            Task.workspace_id == workspace_id, Task.agent_team_id == team_id
        )
        ...
        tasks = list(
            self._session.scalars(
                task_statement.order_by(
                    Task.priority.desc(), Task.updated_at.desc(), Task.id.asc()
                ).limit(limit)
            ).all()
        )
        ...
        latest_events = latest_events_by_run(
            self._session, workspace_id, [run for runs in runs_by_task.values() for run in runs]
        )
...
class TeamProjectDashboardRepository:
    ...
    def steps_by_task(self, workspace_id: UUID, task_ids: list[UUID]) -> dict[UUID, list[TaskStep]]:
        if not task_ids:
            return {}
    ...
    def runs_by_task(self, workspace_id: UUID, task_ids: list[UUID]) -> dict[UUID, list[AgentRun]]:
        if not task_ids:
            return {}
    ...
    def latest_messages(self, workspace_id: UUID, task_ids: list[UUID]) -> dict[UUID, TaskMessage]:
        if not task_ids:
            return {}
...
def _delivery_summary(
    task: Task, steps: list[TaskStep], artifacts: list[Artifact]
) -> dict[str, object]:
...
def _delivery_status(
    task: Task, *, artifact_count: int, missing_expected: int, pending_review: int, rejected: int
) -> str:
...
def _blocked_reasons(task: Task, steps: list[TaskStep], delivery: dict[str, object]) -> list[str]:
    reasons: list[str] = []
...
def _recommended_actions(
    task: Task, blockers: list[str], delivery: dict[str, object], runs: list[AgentRun]
) -> list[str]:
    actions: list[str] = []
...
def _summary(
    items: list[dict[str, object]], *, limit: int, include_completed: bool
) -> dict[str, object]:
...
def _risk_level(task: Task, blockers: list[str], delivery: dict[str, object]) -> str:
...
def _active_run_phase_counts(
    runs: list[AgentRun], latest_events: dict[UUID, RunEvent]
) -> dict[str, int]:
    return _counts(
