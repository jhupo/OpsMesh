...
- Core lifecycle, Redis pooling, machine-aware defaults, database transaction retry helpers, structured log context, split health probes, reusable maintenance runner, HTTP metrics, domain error mapping, production config guardrails, and Redis distributed locks are implemented; feature flags still need completion.
...
- [ ] Add starvation prevention for long-waiting lower-priority work.
