...
- Core lifecycle, Redis pooling, machine-aware defaults, database transaction retry helpers, structured log context, split health probes, reusable maintenance runner, HTTP metrics, and domain error mapping are implemented; distributed locks and feature flags still need completion.
