...
状态：后端实现，2026-09-20。适用于 API、外部插件消息和后台执行；前端权限配置页面不在本次范围内。当前发布门禁修复见插件接入记录，新版尚未部署验收。
...
资源发布审批没有任务归属，单独按工作区管理员权限读取和决策；公开市场读取只开放审核后公开的
listing，安装使用冻结快照，不读取发布者的私有 Agent。市场安装计数由安装服务原子更新。

预算拦截按完整工作区账本计算，包括其他用户的私有任务和已删除任务的账单；不能把用户可见的
明细小计用于工作区额度。该内部汇总仍限定工作区，明细查询继续执行资源过滤。

归档恢复演练在独立数据库 Session 和可回滚的临时事务中创建一次性工作区，避免修改源请求的
资源身份。正常导入器的提交只结束内部保存点；演练完成或失败后回滚临时数据库数据，清理新写入
文件，成功证据记录在已授权的源工作区。它不会授予调用者对其他现存工作区的访问权限。
