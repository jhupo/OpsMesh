...
| R3 | Agent/provider/session/mailbox/tools/MCP | 进行中 | Mailbox 命令、校验、读取、收件箱和汇总已分别收敛到 `domains/agents/messages/service.py` 与 `queries.py`，删除 5 个唯一调用的 Mixin 碎片；`test_agent_messages_api.py` 全部通过。Provider/session/tools/MCP 仍待逐项核对 |
| R4 | 任务/Run/工作流职责 | 待实施 | |
