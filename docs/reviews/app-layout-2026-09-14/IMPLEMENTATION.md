...
| R4 | 任务/Run/工作流职责 | 进行中 | 计划模板已提升到 `workflows/templates`；任务控制与原 operations 已收敛到 `tasks/control`，任务管理与交接归入 `tasks/collaboration`，执行诊断/载荷归入 `tasks/observation`，任务转移及脱敏交接包拆到 `tasks/collaboration/{transfers,transfer_package}.py`，任务 CRUD、Task 状态、Step 状态分别归入 `tasks/service.py`、`tasks/state.py`、`tasks/steps.py`，Run 状态合同并入 `runs/state.py`，旧 `tasks/operations`、`management`、`execution`、`status.py`、`step_service.py`、`step_status.py` 以及 `runs/status.py` 已删除。模板生成、成员匹配、条件校验、Step 物化、转移和 Run 生命周期仍需继续做场景验证。 |
