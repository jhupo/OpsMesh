...
| R5 | Workspace/team/project/archive/data lifecycle | 进行中 | Workspace 原 `domains` 子包已按方案归并为 `extensions`；全量归档与导入导出已从 `projects/{exports,imports}` 收敛到 `data_transfer`，其中归档/仓储/序列化归入根模块，导入实现归入 `data_transfer/importers`；Workspace 生命周期已从 `tenants/lifecycle` 收敛到 `data_lifecycle`，动作、诊断、调度分别合并为稳定边界模块，旧嵌套目录与空包已删除。项目文件、归档导入导出、运行 I/O、生命周期调度和配额场景通过；团队执行/人工控制边界仍待继续核对 |
