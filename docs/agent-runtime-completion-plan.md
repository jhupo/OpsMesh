...
| 6.3 | P0 | Enforce domain, IP, DNS, protocol, and port egress rules through a controlled gateway | Done | `add-runtime-egress-policy` | Runtime creation normalizes an explicit egress policy, rejects unsafe template/network combinations, and records the gateway boundary; direct container networking remains disabled unless the reviewed policy permits it |
| 6.4 | P0 | Enforce non-root execution, seccomp/AppArmor policy, read-only rootfs, and minimal capabilities | Done | `harden-runtime-sandbox-defaults` | Docker requests always carry the platform hardening profile, and runtime metadata records the effective non-root, read-only, seccomp/AppArmor, capability-drop, and tmpfs controls |
| 6.5 | P0 | Enforce CPU, memory, disk, process, wall-time, command-output, and concurrency limits | Done | `complete-runtime-resource-limits` | Limits are validated before launch, enforced by Docker/command execution and scheduler state, and violations produce bounded run/security/audit evidence with cleanup |
| 6.6 | P0 | Complete command authorization, risk review, timeout, termination, and audit | Done | `complete-runtime-command-security` | Static command risk checks and platform approval policy run on queue and execution paths; persisted payload tampering, risky commands, timeout, termination, output bounds, and denial evidence fail closed |
| 6.7 | P1 | Add self-hosted worker capability attestation, revocation, and untrusted-state behavior | Done | `harden-self-hosted-runtime-trust` | Signed capability reports are fingerprinted and time-bounded, unsigned/invalid reports stay `untrusted`, host isolation is never claimed from a report alone, explicit verified-isolation runs are rejected on unverified machines, and revocation blocks future authentication/claims |
...
| 7.2 | P0 | Add focused recovery and denial scenarios | Done | `add-agent-runtime-recovery-e2e` | `test_agent_runtime_recovery_e2e.py` proves forced duplicate delivery executes a run once, wall-time expiry records bounded failure evidence, frozen network-policy denial fails before model execution with runtime/security evidence, saturated runtime-space quota leaves no reservation, and stale worker runs transition to failed with recovery evidence; existing queue and worker recovery tests cover retry/dead-letter and approved-tool replay protection |
| 7.3 | P1 | Integrate runtime logs, metrics, tracing, audit, and cost events | Done | `complete-agent-runtime-operations-evidence` | Run events already carry the worker trace context and normalized cost records; the audit boundary now derives `agent_run_id` for run targets and merges the active trace metadata through the same redaction path. `test_agent_runtime_operations_evidence.py` verifies one Worker execution has a shared trace ID across RunEvent, ModelUsageRecord, and AuditEvent, with the audit row linked by the durable run foreign key |
| 7.4 | P1 | Replace obsolete runtime prompts, flow diagrams, architecture diagrams, and status claims | Done | `update-agent-runtime-architecture-docs` | `agent-runtime-architecture.md` is the current provider-neutral boundary/lifecycle reference; README and backend architecture links and provider/runtime status claims are aligned, stale runtime-manager references are removed, and empty obsolete prompt/asset directories are deleted |
