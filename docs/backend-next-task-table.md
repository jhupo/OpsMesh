...
| 4 | P0 | Multi-agent project execution loop | In progress | Reusable team org chart API, team execution overview API with staffing-gap recommendations, task risk scoring, team-level recommended actions, delivery-health scoring, team bottleneck diagnostics, metadata-only intervention plans, team project-dashboard API for project progress/risk/delivery rollups and phase-count visibility, team command center API, command-center action-plan dry-run/apply with source/action filtering and de-duplicated team operator actions, optional team-scoped run enqueue after command-center apply, execution-loop task finalization dry-run/apply for PM-approved tasks, unified execution-loop iteration endpoint that finalizes PM-approved work before applying remaining team actions and enqueueing runs, pollable execution-loop status API for idle/needs-attention/ready-to-finalize state, unified task control API for pause/resume/add-instruction/create-correction/cancel with worker lease cancel-request lifecycle markers, task control-diagnostics API for pause/resume/correction side effects and worker cancel-request visibility, task execution-status monitor API with current focus/recommended actions/recent events and run activity phase summaries, precise run lifecycle events for worker claim/context build/model request/model response/model failure, team execution overview phase-count rollups, cursor-based task event feed API and SSE `task.events` deltas, task interaction transcript API with participants/message context/pagination, task collaboration-state API for manager/specialist/handoff/acceptance protocol rollups, task delivery-review API with expected artifact coverage/final output/review actions, task delivery-decision API for approve/request-changes/reject with correction handoff, handoff/manager-queue team action payload plans, team-level manager-review/requeue/downstream operator actions, task execution diagnostics with step-level handoff state, workspace/team-filtered handoff queue, project-plan quality diagnostics, correction lifecycle diagnostics, task execution timeline API, manager handoff/acceptance diagnostics, workspace/team-filtered manager queue API, and domain-specific observation status cards for AIGC/novel/research/software tasks are implemented; operator actions now requeue blocked steps, schedule downstream handoff work, reassign specialist work, and request manager review steps |
| 5 | P1 | MCP skill lifecycle and tool permission hardening | In progress | Private/public skill install lifecycle and version snapshots exist; skill installs now support impact analysis, upgrade history, rollback, disable, availability checks, per-agent tool-policy diagnostics, workspace-level tool-policy matrix, MCP tool-call execution audit boundary checks, stale MCP health-check blocking, health-check refresh reporting, governance dry-run/apply health-refresh requests with pending execution blocking, per-run/hourly MCP tool call limits with catalog policy summaries, workspace capability governance summaries with recommended operator actions, and dry-run/apply governance action remediation for disabling unusable skill installs or clearly blocked MCP servers without exposing secrets |
...
| 7 | P1 | Operations control plane expansion | Done | Runtime/container quota dashboard API, backlog/failure/latency aggregates, run activity phase aggregates with oldest active run diagnostics, blocked-step explanations, workspace health snapshot API with persisted health history and trend comparison, worker maintenance auto-samples explicitly opted-in workspace health snapshots, and metadata-only operator responses are covered by operations API and tests |
| 8 | P1 | Workspace data lifecycle | In progress | Import/export restore path, artifact versioning, archive job download, archive integrity verification, and file access audit exist; lifecycle diagnostics reports backup/retention/restore-drill readiness, backup schedule due/overdue state, and automation visibility for latest scheduled jobs, active exports, recent lifecycle events, skipped reasons, retention auto-apply status, and scheduled restore-drill status; worker maintenance now enqueues due scheduled archive backups, can run explicitly opted-in retention auto-apply, and can execute due restore drills against the latest successful archive with audit evidence; recovery readiness summarizes archive export/import health, backup integrity checks, backup freshness, backup coverage scoring, restore test history, restore drill evidence, import conflict preview history, overdue backup schedule warnings, and restore recommendations; recovery-readiness actions now support dry-run/apply archive export remediation and restore-import-test remediation with active-job dedupe, audit evidence, and redacted metadata; retention preview/apply identifies scoped candidates, returns recommended operator actions, soft-deletes expired files, requires successful backups by default, and audits operator actions |
