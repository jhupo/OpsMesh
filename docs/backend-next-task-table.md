...
| 5 | P1 | Add one isolated remote validation script/table entry for the above closures | Done | `scripts/remote-backend-validation.sh` creates disposable Docker network/Postgres/Redis/test containers, runs targeted pytest/ruff with overrideable arguments, documents the workflow, and avoids `chaincloud-postgres` |
