...
| 4 | P0 | Multi-agent project execution loop | In progress | Reusable team org chart API, team execution overview API, task execution diagnostics, project-plan quality diagnostics, correction lifecycle diagnostics, task execution timeline API, manager handoff/acceptance diagnostics, and workspace manager queue API are implemented; operator actions now requeue blocked steps, reassign specialist work, and request manager review steps |
| 5 | P1 | MCP skill lifecycle and tool permission hardening | In progress | Private/public skill install lifecycle and version snapshots exist; skill installs now support impact analysis, upgrade history, rollback, disable, availability checks, per-agent tool-policy diagnostics, workspace-level tool-policy matrix, and workspace capability governance summaries without exposing secrets |
...
| 8 | P1 | Workspace data lifecycle | In progress | Import/export restore path, artifact versioning, archive job download, and file access audit exist; lifecycle diagnostics reports backup/retention readiness; recovery readiness now summarizes archive export/import health; retention preview/apply identifies scoped candidates, soft-deletes expired files, requires successful backups by default, and audits operator actions |
