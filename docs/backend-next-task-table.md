...
| 7 | P1 | Operations control plane expansion | Done | Runtime/container quota dashboard API, backlog/failure/latency aggregates, blocked-step explanations, workspace health snapshot API with persisted health history, and metadata-only operator responses are covered by operations API and tests |
