...
| 2 | P0 | Provider/model operations closure | Done | `/workspaces/{workspace_id}/operations/model-providers` exposes per-agent provider/model/protocol readiness, budget/limit summaries, health/failure diagnostics, redacted provenance, active-run snapshots, and validated fallback candidates for OpenAI, OpenAI-compatible, and Anthropic paths; targeted coverage in `test_model_provider_operations.py` |
...
| --- | --- | --- | --- | --- |
| 1 | P0 | Dedicated isolated runtime image and MCP SDK capability probe | Done | `Dockerfile.runtime` installs only `opsmesh-runtime` plus MCP SDK, runs non-root, uses `/workspace`, and exposes `python -m opsmesh_runtime.mcp_stdio_client --check`; `scripts/check_runtime_image.py` verifies the exact image user, no-network/read-only/cap-drop execution, and SDK capability in both delivery and release gates; Docker adapter fails closed before launching a server |
| 2 | P0 | Official SDK stdio integration proof | Done | A real FastMCP fixture verifies SDK initialization and tool execution locally and inside the built runtime image through `scripts/check_runtime_image.py`; the release gate probes the exact candidate digest before publication |
...
| 4 | P1 | MCP runtime security and recovery evidence | In progress | Targeted tests cover strict completion payloads, exact-result idempotency, conflicting-result rejection, retryable/permanent control-plane error classification, full-session timeout, process failure, claimed-job rediscovery, result replay, interrupted execution, transient Docker stdin credential injection, local self-hosted environment references, response/credential redaction, plaintext remote URL denial, and workspace denial; `scripts/self-hosted-connector-smoke.sh` provides repeatable packaged validation against a deployed control plane, with staging execution still required |

## Observability, Audit, and Cost Closure

| Order | Priority | Task | Status | Acceptance Evidence |
| --- | --- | --- | --- | --- |
| 1 | P0 | Logs, metrics, and distributed tracing | Done | Official OpenTelemetry SDK exports structured API/worker logs and W3C-correlated FastAPI, HTTPX, SQLAlchemy, Redis, queue, worker, model, and tool spans through OTLP; the official Prometheus client exposes bounded route-template and governance metrics; the pinned Prometheus/Loki/Tempo/Collector/Grafana stack includes retention, correlations, service graphs, alerts, and a fail-closed smoke path |
| 2 | P0 | Audit integrity operations | Done | Workspace-serialized SHA-256 chains, PostgreSQL WORM triggers, scheduled and on-demand verification, durable integrity snapshots, admin-scoped APIs, Prometheus state gauges, dashboard panels, and invalid/stale alerts are implemented and covered by targeted tests |
| 3 | P0 | Model usage and cost accounting | Done | Workspace pricing versions, immutable per-attempt price snapshots, normalized/redacted token usage, database-side summaries, per-currency monthly budgets, warn/block enforcement, audited admin APIs, dashboard gauges, and unpriced/missing/budget alerts are implemented and covered by worker/API/tenant tests |
