...
| 3 | P1 | Add MCP permission remediation evidence to operations/timeline APIs | In progress | Capability governance apply now records redacted MCP health refresh evidence, team runtime timeline surfaces relevant MCP governance remediation events for MCP servers used by team members, and governance apply can remove unallowed agent MCP tool references with audit evidence; remaining work is server/tool-side permission repair actions |
