...
| 3 | P1 | MCP tool permission production closure | In progress | Workspace/team/agent tool-policy matrix can explain, dry-run, apply, and audit permission remediation; remote Streamable HTTP/SSE and isolated stdio execution now delegate to the official MCP Python SDK; runtime image packaging, self-hosted connector implementation, and live server validation remain |
