...
| 8 | P1 | Workspace data lifecycle | In progress | Import/export restore path, artifact versioning, archive job download, and file access audit exist; lifecycle diagnostics reports backup/retention readiness; recovery readiness now summarizes archive export/import health, backup freshness, and restore recommendations; retention preview/apply identifies scoped candidates, soft-deletes expired files, requires successful backups by default, and audits operator actions |
