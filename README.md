...
| Knowledge registry and vector/hybrid retrieval | Partially implemented: workspace-scoped URL/workspace-file registration plus bounded asynchronous workspace-file ingestion are complete alongside three-layer memory, hybrid retrieval, and authorized context injection; isolated URL fetching and citations remain planned |
...
| Knowledge service | Workspace-scoped source registration, idempotent workspace-file ingestion, three-layer memory, Postgres full-text, pgvector/HNSW similarity, weighted hybrid ranking, grant-scoped context injection, lifecycle policy, and evidence are implemented; isolated URL fetching and citation contracts remain planned |
